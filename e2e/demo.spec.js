...
test('Demo completo: cadastro → rodadas → resultados → play-in → final', async ({ page }) => {
    ...
    // ═══════ 12-15. BRACKET: PLAY-IN → QUARTAS → SEMIS → 3º LUGAR → FINAL ═══════
    ...
    async function scoreSetMatch(page, matchId, sets) {
        await page.evaluate((id) => window.__app.openEditBracketModal(id), matchId);
        await page.waitForSelector('#edit-bracket-modal:not(.hidden)', { timeout: 5000 });
    ...
    const semisSets = [
        [[21, 18], [21, 15]],            // 2×0
        [[19, 21], [22, 20], [15, 13]],  // 2×1
    ];
    for (let i = 0; i < knockoutIds.semis.length; i++) {
        await scoreSetMatch(page, knockoutIds.semis[i], semisSets[i]);
    ...
    if (terceiroId) {
        await scoreSetMatch(page, terceiroId, [[15, 21], [21, 19], [13, 15]]); // 1×2
        console.log('  3º lugar ✓');
        await page.waitForTimeout(2000);
    }

    // Final — melhor de 3 sets
    await scoreSetMatch(page, knockoutIds.final[0], [[21, 19], [18, 21], [16, 14]]); // 2×1
    console.log('  Final ✓');
